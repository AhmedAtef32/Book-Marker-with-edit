...
function display() {
  ...
  for (var i = 0; i < sitesList.length; i++) {
    blackBox += `     <tr>
                        ...
                        <td><button class="btn  btn-success Edit" onclick="edit(${i})"><i class="fa-solid fa-pen-to-square pe-1"></i>Edit</button></td>
                    </tr>`;
  }
...
function edit(editindex) {
  ...
  sitename.value = sitesList[editindex].siteName;
  siteurl.value = sitesList[editindex].siteUrl;
  ...
  if (validation(sitename) && validation(siteurl)) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        sitesList[updateIndex].siteName = sitename.value;
        sitesList[updateIndex].siteUrl = siteurl.value;
...
function validation(input) {
  var regex = {
    ...
    siteuRL:
      /^(https:\/\/|http:\/\/){1}.{1,}\.[a-zA-Z0-9()]{2,6}\b([-a-zA-Z0-9()!@:%_\+.~#?&\/\/=]*)$/,
